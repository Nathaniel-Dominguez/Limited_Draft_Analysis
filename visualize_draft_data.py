...
def plot_mana_curve(analysis, output_dir):
    """Plot the average mana curve from the draft analysis"""
    ...
    # Plot the mana curve as a bar chart
    x = [pair[0] for pair in mana_values]
    y = [pair[1]/100 for pair in mana_values]  # Normalize by number of decks
    ...
    plt.title('Average Mana Curve Across Simulated Decks')
...
def plot_color_distribution(analysis, output_dir):
    """Plot the color distribution from the draft analysis"""
    ...
    plt.bar(colors_df['Color'], colors_df['Count'], color=bar_colors, edgecolor='black')
    ...
    for i, v in enumerate(colors_df['Count']):
...
def plot_rarity_distribution(analysis, output_dir):
    """Plot the rarity distribution from the draft analysis"""
    ...
    plt.pie(rarity_df['Count'], labels=rarity_df['Rarity'], autopct='%1.1f%%', 
            colors=colors, startangle=90, shadow=True)
    
    # Add title
    plt.title('Rarity Distribution Across All Simulated Decks')
